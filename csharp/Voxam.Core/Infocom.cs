...
///
/// Infocom's story files carry no iFiction record, since they shipped
/// two decades before the Treaty of Babel, but the catalog is finite
/// and closed, so their identities are simply known. This table maps
/// every legacy IFID in Andrew Plotkin's Obsessively Complete Infocom
/// Catalog (https://eblong.com/infocom/, its catalog.json, retrieved
/// 2026-08-23) to the title the catalog gives it, keyed exactly as
/// <see cref="Babel"/> computes those IFIDs from the header numbers.
///
/// Two footnotes. Release 5 serial XXXXXX is omitted: two different
/// games, Suspended and Zork 1, both shipped under it, so that
/// identity is genuinely ambiguous and the table honestly cannot name
/// it. And the Leather Goddesses beta r59-000001 is keyed with its
/// checksum, because a serial outside the trusted forms earns one
/// (Babel: The IFID for a legacy Z-code story file).
///
/// The table is the reference's own, carried over entry for entry:
/// the two are the same catalog and must answer alike.
/// </summary>
public static class Infocom
{
    /// <summary>The catalog's name for an IFID, or null for the unknown.</summary>
    public static string? Title(string? identity) =>
        identity is not null && Titles.TryGetValue(identity, out var named) ? named : null;

    private static readonly FrozenDictionary<string, string> Titles =
        new Dictionary<string, string>(StringComparer.Ordinal)
        {
            ...
            ["ZCODE-19-870722"] = "Nord and Bert Couldn't Make Head or Tail of It",
            ["ZCODE-20-870722"] = "Nord and Bert Couldn't Make Head or Tail of It",
