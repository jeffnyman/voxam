...
def test_reads_identity_of_every_fixture(version: int, release: int) -> None:
    header = fixture_story(version).header
...
def test_declared_length_never_exceeds_actual_size(version: int) -> None:
    story = fixture_story(version)
...
@pytest.mark.parametrize("version", ALL_VERSIONS)
def test_every_fixture_passes_verification(version: int) -> None:
    header = fixture_story(version).header

    assert_that(header.verify()).is_true()


def test_detects_corruption_through_verification() -> None:
    data = bytearray(fixture_story(3).data)
...
def test_initial_program_counter_lies_within_the_file(version: int) -> None:
    story = fixture_story(version)
...
def test_version_6_stores_a_packed_routine_address() -> None:
    header = fixture_story(6).header

    assert_that(header.main_routine_packed_address).is_greater_than(0)


def test_version_6_refuses_an_initial_program_counter() -> None:
    header = fixture_story(6).header
...
def test_other_versions_refuse_a_packed_routine_address() -> None:
    header = fixture_story(5).header
