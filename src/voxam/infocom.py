...
Infocom's story files carry no iFiction record -- they shipped two
decades before the Treaty of Babel -- but the catalog is finite
...
voxam.babel computes those IFIDs from the header numbers.

Two footnotes. Release 5 serial XXXXXX is omitted: two different
games -- Suspended and Zork 1 -- both shipped under it, so that
identity is genuinely ambiguous and the table honestly cannot
name it. And the Leather Goddesses beta r59-000001 is keyed with
its checksum, because a serial outside the trusted forms earns
one (Babel: The IFID for a legacy Z-code story file).
"""
...
def title(identity: str | None) -> str | None:
    """The catalog's name for an IFID, or None for the unknown."""

    if identity is None:
        return None

    return TITLES.get(identity)
